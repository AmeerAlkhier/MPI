...
if(id != 0)
    ...
    {//Master recieving
        ...
        for(int j=1;j<nproc;++j)
        {
            ...
            //for(int i=startval;i<endval; ++i)
            MPI_Recv(&img_array[startval][0], (endval-startval)*imgY, MPI_INT, id_from, 2, MPI_COMM_WORLD, &status);
        ...
        // we have to colour our dataset. Actually, the members of the Mandelbrot set
        ...
        for(int i=0;i<imgX;i++)
        {
            ...
            for(int j=0;j<imgY;j++)
            {
                if( (img_array[i][j] < 256) ) // we go from black to red in this range
                    {
                    myfile << img_array[i][j] << " 0 0"; // (int)(84*pow(img_array[i][j],0.2)) 0"; //myfile << img_array[i][j] << " 0 0";
                    }
