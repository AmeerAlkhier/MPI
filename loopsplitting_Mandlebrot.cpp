...
int converges(double cx,double cy)
 {
    ...
    double new_zx=0;
    double zy=0;
    // we iterate until max. iteration count iter_n, or until z^2 (complex!) runs over
    ...
    if(id == 0)
        { //Master
                ...
                
            {
                ...
                MPI_Recv(&img_array[i][0], imgY, MPI_INT, id_from, 2, MPI_COMM_WORLD, &status);

            }

            // we get another time at this point, so we can calculate the elapsed time for
            //the calculation
            ...
            // the outer points are represented by assigning colours to their iteration
            ...
            for(int i=0;i<imgX;i++)
            {
                ...
                /* 
                    // we could refine our palette for more resolution, more iteration-step
                    images
                    128:
            ...
            {   //Slave
                ...
                for(int i=id-1;i<imgX;i += (nproc-1))
                    ...
                    {
                        cy=yMin; 
                        // at every new step in X direction,
